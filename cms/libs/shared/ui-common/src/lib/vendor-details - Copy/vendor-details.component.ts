...
constructor(
 ...
 private readonly cdr: ChangeDetectorRef,
  ...
  setClinicFormValues() {
    this.clinicForm.controls['providerName'].setValue(this.clinicDetails.vendorName);
    this.clinicForm.controls['firstName'].setValue(this.clinicDetails.firstName);
    this.clinicForm.controls['lastName'].setValue(this.clinicDetails.lastName);
    this.clinicForm.controls['tinNumber'].setValue(this.clinicDetails.tin);
    this.clinicForm.controls['npiNbr'].setValue(this.clinicDetails.npiNbr);
    this.clinicForm.controls['parentVendorId'].setValue(this.clinicDetails.parentVendorId);
    this.clinicForm.controls['vendorTypeCode'].setValue(this.clinicDetails.vendorTypeCode);
    if (this.clinicDetails.preferredFlag != null) {
      let flag = this.clinicDetails.preferredFlag == 'Y' ? true : false
      this.clinicForm.controls['isPreferedPharmacy'].setValue(flag);
    }
    else {
      this.clinicForm.controls['isPreferedPharmacy'].setValue(this.clinicDetails.preferredFlag);
    }
  }
...
fillFormData(){
  ...
  validateClinicForm() {
    ...
    if (this.providerType == this.vendorTypes.MedicalProviders || this.providerType == this.vendorTypes.DentalProviders) {
      if (!this.clinicNameNotApplicable) {
        this.clinicForm.controls['providerName'].setValidators([Validators.required, Validators.maxLength(500)]);
        this.clinicForm.controls['providerName'].updateValueAndValidity();
      }
      if (!this.firstLastNameNotApplicable) {
        this.clinicForm.controls['firstName'].setValidators([Validators.required]);
        this.clinicForm.controls['lastName'].setValidators([Validators.required]);
        this.clinicForm.controls['firstName'].updateValueAndValidity();
        this.clinicForm.controls['lastName'].updateValueAndValidity();
      }
    }
    else if(this.providerType == this.vendorTypes.Manufacturers) {
      this.clinicForm.controls['providerName']
      .setValidators([
        Validators.required,Validators.required,Validators.pattern('^[A-Za-z ]+$')
      ]);
      this.clinicForm.controls['providerName'].updateValueAndValidity();
    ...
    let mailCode = this.clinicForm.controls['mailCode'].value;
    this.validateMailCode()
 
    if (mailCode) {
               this.clinicForm.controls['addressLine1']
        .setValidators([
          Validators.required,
        ]);
      this.clinicForm.controls['addressLine1'].updateValueAndValidity();

      this.clinicForm.controls['city']
        .setValidators([
          Validators.required,Validators.required,Validators.pattern('^[A-Za-z ]+$')
        ]);
      this.clinicForm.controls['city'].updateValueAndValidity();

      this.clinicForm.controls['state']
        .setValidators([
          Validators.required,
        ]);
      this.clinicForm.controls['state'].updateValueAndValidity();

      this.clinicForm.controls['zip']
        .setValidators([
          Validators.required,Validators.required,Validators.pattern('^[A-Za-z0-9 \-]+$')
        ]);
      this.clinicForm.controls['zip'].updateValueAndValidity();

      if (this.providerType == this.vendorTypes.Manufacturers) {
        this.clinicForm.controls['nameOnCheck'].setValidators([
          Validators.nullValidator,
        ]);
        ...
        this.clinicForm.controls['nameOnEnvolop'].setValidators([
          Validators.nullValidator,
        ]);
        this.clinicForm.controls[
          'nameOnEnvolop'
    ...
    if(this.providerType != this.vendorTypes.Manufacturers)
    {
      this.clinicForm.controls['paymentMethod']
        .setValidators([
          Validators.required,
        ]);
      this.clinicForm.controls['paymentMethod'].updateValueAndValidity();

    }

    if (this.providerType == this.vendorTypes.InsuranceVendors) {

      this.clinicForm.controls['paymentRunDate']
        .setValidators([
          Validators.required,
        ]);
      this.clinicForm.controls['paymentRunDate'].updateValueAndValidity();

      this.clinicForm.controls['isAcceptCombinedPayment']
        .setValidators([
          Validators.required,
        ]);
      this.clinicForm.controls['isAcceptCombinedPayment'].updateValueAndValidity();

      this.clinicForm.controls['isAcceptReports']
        .setValidators([
          Validators.required,
        ]);
      this.clinicForm.controls['isAcceptReports'].updateValueAndValidity();
    }
