...
export class SystemInterfaceDashboardPageComponent implements OnInit,OnDestroy {
  ...
  // 
    ...
    allWidgetsconfigSubscriptionItems: GridsterConfig = {
      itemChangeCallback: SystemInterfaceDashboardPageComponent.itemChange,
        ...
        response.filter((element: any) => {
          ...
          SystemInterfaceDashboardPageComponent.dashBoardContentData = []   
           ...
           response.forEach((widg: any) => {    
              ...
              SystemInterfaceDashboardPageComponent.dashBoardContentData.push(widg)
 ...
 this.dashboardContentUpdate$.subscribe((response) => {
  ...
  static itemChange(
    ...
    const dashBoardData = SystemInterfaceDashboardPageComponent.dashBoardContentData;
    let changedWidget = dashBoardData.filter(
      (x: any) => x.widgetProperties.componentData['id'] == item['id']
    );
    SystemInterfaceDashboardPageComponent.updatedWidgets.forEach((widg: any) => {
      if (widg[0]?.widgetProperties?.componentData.id == item['id']) {
        SystemInterfaceDashboardPageComponent.updatedWidgets.splice(widg, 1);
      }
    });
    if (changedWidget[0]?.widgetId) {
      SystemInterfaceDashboardPageComponent.updatedWidgets.push(changedWidget);
    }
  }
