...
import { ReminderSnackBarTemplateComponent } from '../reminder-snackbar-template/reminder-snackbar-template.component';
@Component({
...
export class ReminderNotificationSnackBarComponent implements OnInit {
  ...
  remindersUnViewedCount$ =
    this.signalrEventHandlerService.remindersUnViewedCount$;
  alertText = '';
  entityId = '';
  vendorTypeCode = '';
  entityTypeCode = '';
  ...
  medicalProviderSearchLoaderVisibility$ =
    this.financialVendorFacade.medicalProviderSearchLoaderVisibility$;
  providerSearchResult$ = this.financialVendorFacade.searchProvider$;
  clientSearchLoaderVisibility$ =
    this.financialRefundFacade.clientSearchLoaderVisibility$;
  ...
  reminderSnackBarSubscribe() {
    ...
    this.reminderSnackBar$.subscribe((res: any) => {
      debugger;
      this.dynamicComponent = this.viewContainerRef.createComponent(ReminderSnackBarTemplateComponent);
      this.dynamicComponent.instance.message =res
      this.snackbarMessage = res;
      this.dynamicComponent.instance.entityName = res.alertExtraProperties.EntityName;
      this.dynamicComponent.instance.entityId = res.alertExtraProperties.EntityId;
      this.dynamicComponent.instance.vendorTypeCode = res.alertExtraProperties.VendorTypeCode;
      this.dynamicComponent.instance.entityTypeCode = res.alertExtraProperties.EntityTypeCode;
      this.dynamicComponent.instance.alertId = res.alertExtraProperties.AlertId;
      const repeatTime = res.alertExtraProperties.RepeatTime;
      const dueDate = new Date(
        this.intl.formatDate(
          res.alertExtraProperties.AlertDueDate,
          this.dateFormat
        )
      );
      ...
      this.reminderNotificationSnackbarService.manageSnackBar(
      ...
      next: (res) => {
        ...
        if (res) {
          ...
          const notificationRef: NotificationRef =    this.notificationService.show({
            ...
            this.dynamicComponent.instance.ignore
                .subscribe(() => alert('here'));
