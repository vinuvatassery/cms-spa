...
export class PharmacyClaimsAllPaymentsListComponent
  implements OnInit, OnChanges
{
  ...
  gridPharmacyClaimsAllPaymentsData$ =
    this.gridPharmacyClaimsAllPaymentsDataSubject.asObservable();
    ...
    return [
      {
        ...
        text: 'Edit Claim',
        disabled: ![PaymentStatusCode.Denied].includes(
          dataItem.paymentStatusCode
        ),
        icon: 'edit',
        click: (data: any): void => {
  ...
  private loadPharmacyClaimsAllPaymentsListGrid(): void {
    const params = new GridFilterParam(
      this.state.skip,
      this.state.take,
      this.sortValue,
      this.sortType,
  ...
  navToReconcilePayments(event: any) {
    this.route.navigate(
      ['/financial-management/pharmacy-claims/payments/reconcile-payments'],
  ...
  selectionChange(dataItem: any, selected: boolean) {
    ...
    this.selectedAllPaymentsList = {
      ...
      PrintAdviceLetterUnSelected: this.unCheckedProcessRequest,
      PrintAdviceLetterSelected: this.checkedAndUncheckedRecordsFromSelectAll,
      print: true,
      batchId: null,
      currentPrintAdviceLetterGridFilter: null,
      requestFlow: 'print',
    };
    if (this.selectAll) {
      if (this.unCheckedProcessRequest?.length > 0) {
        this.sendReportCount =
          this.totalGridRecordsCount - this.unCheckedProcessRequest?.length;
        this.recordCountWhenSelectallClicked = this.sendReportCount;
      } else {
        this.recordCountWhenSelectallClicked = selected
          ? this.recordCountWhenSelectallClicked + 1
          : this.recordCountWhenSelectallClicked - 1;
        this.sendReportCount = this.recordCountWhenSelectallClicked;
      }
    } else {
      this.sendReportCount =
        this.selectedAllPaymentsList?.PrintAdviceLetterSelected?.filter(
  ...
  onRecordSelectionUnChecked(dataItem: any) {
    this.unCheckedProcessRequest = this.unCheckedProcessRequest.filter(
      (item: any) => item.paymentRequestId !== dataItem.paymentRequestId
    );
    ...
    const exist = this.checkedAndUncheckedRecordsFromSelectAll?.filter(
    ...
    if (exist === 0) {
      ...
      const recordIndex =
        this.checkedAndUncheckedRecordsFromSelectAll.findIndex(
          (element: any) =>
            element.paymentRequestId === dataItem.paymentRequestId
        );
      if (recordIndex !== -1) {
  ...
  onRecordSelectionChecked(dataItem: any) {
    this.unCheckedProcessRequest.push({
      paymentRequestId: dataItem.paymentRequestId,
      vendorAddressId: dataItem.vendorAddressId,
      selected: true,
    });
  ...
  markAsChecked(data: any) {
    data.forEach((element: any) => {
      ...
      if (
        this.unCheckedPaymentRequest.length > 0 ||
        this.selectedDataIfSelectAllUnchecked.length > 0
      ) {
        const itemMarkedAsUnChecked = this.unCheckedPaymentRequest.find(
          (x: any) => x.paymentRequestId === element.paymentRequestId
        );
        if (
          itemMarkedAsUnChecked !== null &&
          itemMarkedAsUnChecked !== undefined
        ) {
          element.selected = false;
        }
        const itemMarkedAsChecked = this.selectedDataIfSelectAllUnchecked.find(
          (x: any) => x.paymentRequestId === element.paymentRequestId
        );
  ...
  handleAllPaymentsGridData() {
    this.pharmacyClaimsAllPaymentsGridLists$.subscribe(
      (data: GridDataResult) => {
        ...
        if (!this.selectAll) {
          this.pharmacyClaimsAllPaymentsGridLists.forEach((item1: any) => {
            const matchingGridItem =
              this.selectedAllPaymentsList?.PrintAdviceLetterSelected.find(
                (item2: any) =>
                  item2.paymentRequestId === item1.paymentRequestId
              );
            if (matchingGridItem) {
              item1.selected = true;
            } else {
  ...
  handlePageCountSelectionChange() {
    if (!this.selectAll && (this.isPageChanged || this.isPageCountChanged)) {
      ...
      const idsToKeep: number[] =
        this.checkedAndUncheckedRecordsFromSelectAll.map(
          (item: any) => item.paymentRequestId
        );
      // Remove items from selected records based on the IDs from grid data
      for (
        let i =
          this.selectedAllPaymentsList?.PrintAdviceLetterSelected?.length - 1;
        i >= 0;
        i--
      ) {
        if (
          !idsToKeep.includes(
            this.selectedAllPaymentsList?.PrintAdviceLetterSelected[i]
              .paymentRequestId
          )
        ) {
  ...
  pageNumberAndCountChangedInSelectAll() {
    ...
    if (this.selectAll && (this.isPageChanged || this.isPageCountChanged)) {
      ...
      for (const item of this.pharmacyClaimsAllPaymentsGridLists) {
        ...
        const isItemInSecondList = this.unCheckedProcessRequest.find(
          (item2: any) => item2.paymentRequestId === item.paymentRequestId
        );
