...
export class PharmacyClaimsBatchesReconcilePaymentsComponent implements OnInit {
  ...
  checkErrorCount() {
    const datePaymentSentInValidCount =
      this.reconcilePaymentGridUpdatedResult.filter(
        (x: any) => x.datePaymentSentInValid
      );
    const datePaymentRecInValidCount =
      this.reconcilePaymentGridUpdatedResult.filter(
        (x: any) => x.datePaymentRecInValid
      );
    const warrantNumberInValidCount =
      this.reconcilePaymentGridUpdatedResult.filter(
        (x: any) => x.warrantNumberInValid
      );
    this.totalCount =
      datePaymentSentInValidCount.length +
      datePaymentRecInValidCount.length +
      warrantNumberInValidCount.length;
    if (this.totalCount === 0) {
      ...
      this.pageValidationMessage =
        this.totalCount +
        ' validation errors found, please review each page for errors. ';
    }
  }

  paymentMethodSubscription() {
    this.paymentMethodLovSubscription = this.paymentMethodType$.subscribe(
      (data) => {
  ...
  columnChange(event: ColumnVisibilityChangeEvent) {
    ...
    this.columnChangeDesc = columnsRemoved
      ? 'Columns Removed'
      : 'Default Columns';
  }
  ...
  assignRowDataToMainList(dataItem: any) {
    let ifExist = this.reconcilePaymentGridUpdatedResult.find(
      (x: any) => x.paymentRequestId === dataItem.paymentRequestId
    );
    if (ifExist !== undefined) {
      this.reconcilePaymentGridUpdatedResult.forEach(
        (item: any, index: number) => {
          if (item.paymentRequestId === ifExist.paymentRequestId) {
            this.reconcilePaymentGridUpdatedResult[index].paymentRequestId =
              dataItem.paymentRequestId;
            this.reconcilePaymentGridUpdatedResult[index].clientId =
              dataItem.clientId;
            this.reconcilePaymentGridUpdatedResult[
              index
            ].paymentReconciledDate = dataItem.paymentReconciledDate;
            this.reconcilePaymentGridUpdatedResult[index].paymentSentDate =
              dataItem.paymentSentDate;
            this.reconcilePaymentGridUpdatedResult[index].checkNbr =
              dataItem.checkNbr;
            this.reconcilePaymentGridUpdatedResult[index].comments =
              dataItem.comments;
            this.reconcilePaymentGridUpdatedResult[
              index
            ].datePaymentRecInValid = dataItem?.datePaymentRecInValid;
            this.reconcilePaymentGridUpdatedResult[
              index
            ].datePaymentRecInValidMsg = dataItem?.datePaymentRecInValidMsg;
            this.reconcilePaymentGridUpdatedResult[
              index
            ].datePaymentSentInValid = dataItem?.datePaymentSentInValid;
            this.reconcilePaymentGridUpdatedResult[
              index
            ].datePaymentSentInValidMsg = dataItem?.datePaymentSentInValidMsg;
            this.reconcilePaymentGridUpdatedResult[index].isPrintAdviceLetter =
              dataItem?.isPrintAdviceLetter;
            this.reconcilePaymentGridUpdatedResult[
              index
            ].tAreaCessationCounter = dataItem?.tAreaCessationCounter;
            this.reconcilePaymentGridUpdatedResult[index].vendorName =
              dataItem?.vendorName;
            this.reconcilePaymentGridUpdatedResult[index].amountPaid =
              dataItem?.amountPaid;
            this.reconcilePaymentGridUpdatedResult[index].paymentMethodCode =
              dataItem?.paymentMethodCode;
            this.reconcilePaymentGridUpdatedResult[index].batchId =
              dataItem?.batchId;
          }
        }
      );
    ...
    this.claimReconcileCount = this.reconcilePaymentGridUpdatedResult.filter(
      (x: any) => x.warrantNumberChanged
    ).length;
    this.isRecordForPrint = this.reconcilePaymentGridUpdatedResult.filter(
      (x: any) => x.isPrintAdviceLetter
    ).length;
    this.hasWarrantNo = this.reconcilePaymentGridUpdatedResult.filter(
      (x: any) => x.checkNbr
    ).length;
  }
  ...
  assignPaymentReconciledDateToPagedList() {
    this.reconcilePaymentGridPagedResult.data.forEach((item: any) => {
      if (
        item.checkNbr !== undefined &&
        item.checkNbr !== '' &&
        item.checkNbr !== null &&
        !item.reconciled
      ) {
        item.paymentReconciledDate =
          this.reconcileAssignValueBatchForm.controls[
  ...
  assignDataFromUpdatedResultToPagedResult(itemResponse: any) {
    itemResponse.data.forEach((item: any, index: number) => {
      let ifExist = this.reconcilePaymentGridUpdatedResult.find(
        (x: any) => x.paymentRequestId === item.paymentRequestId
      );
      if (
        ifExist !== undefined &&
        item.paymentRequestId === ifExist.paymentRequestId
      ) {
        itemResponse.data[index].paymentRequestId = ifExist?.paymentRequestId;
        itemResponse.data[index].clientId = ifExist?.clientId;
        itemResponse.data[index].paymentReconciledDate =
          ifExist?.paymentReconciledDate !== null
            ? new Date(ifExist?.paymentReconciledDate)
            : ifExist?.paymentReconciledDate;
        itemResponse.data[index].paymentSentDate =
          ifExist?.paymentSentDate !== null
            ? new Date(ifExist?.paymentSentDate)
            : ifExist?.paymentSentDate;
        itemResponse.data[index].checkNbr = ifExist?.checkNbr;
        itemResponse.data[index].comments = ifExist?.comments;
        itemResponse.data[index].datePaymentRecInValid =
          ifExist?.datePaymentRecInValid;
        itemResponse.data[index].datePaymentRecInValidMsg =
          ifExist?.datePaymentRecInValidMsg;
        itemResponse.data[index].datePaymentSentInValid =
          ifExist?.datePaymentSentInValid;
        itemResponse.data[index].datePaymentSentInValidMsg =
          ifExist?.datePaymentSentInValidMsg;
        itemResponse.data[index].isPrintAdviceLetter =
          ifExist?.isPrintAdviceLetter;
        ...
        itemResponse.data[index].paymentReconciledDate =
          itemResponse.data[index].paymentReconciledDate !== null
            ? new Date(itemResponse.data[index].paymentReconciledDate)
            : itemResponse.data[index].paymentReconciledDate;
        itemResponse.data[index].paymentSentDate =
          itemResponse.data[index].paymentSentDate !== null
            ? new Date(itemResponse.data[index].paymentSentDate)
            : itemResponse.data[index].paymentSentDate;
        if (
          itemResponse.data[index].checkNbr !== null &&
          itemResponse.data[index].checkNbr !== '' &&
          itemResponse.data[index].checkNbr !== undefined
        ) {
  ...
  gridDataHandle() {
    this.reconcileGridListsSubscription = this.reconcileGridLists$.subscribe(
      (response: any) => {
        if (response.data.length > 0) {
          ...
          this.gridClaimsReconcileDataSubject.next(
  ...
  reconcileDateOnChange(control: any) {
    ...
    const datePaymentReconciled =
      this.reconcileAssignValueBatchForm.controls[control].value;
    if (datePaymentReconciled > this.currentDate) {
      ...
      this.reconcileAssignValueBatchForm.controls[control].setErrors({
        incorrect: true,
      });
    }
  }
  reconcileDateBatch() {
    ...
    if (
      this.reconcileAssignValueBatchForm.controls['datePaymentReconciled']
        .value === null ||
      this.reconcileAssignValueBatchForm.controls['datePaymentReconciled']
        .value === ''
    ) {
  ...
  paymentSentDateBatch() {
    ...
    if (
      this.reconcileAssignValueBatchForm.controls['datePaymentSend'].errors !==
      null
    ) {
      if (
        this.reconcileAssignValueBatchForm.controls['datePaymentSend'].errors[
  ...
  noteBatch() {
    this.reconcileAssignValueBatchForm.markAllAsTouched();
    if (
      this.reconcileAssignValueBatchForm.controls['note'].value === null ||
      this.reconcileAssignValueBatchForm.controls['note'].value === ''
    ) {
  ...
  dateChangeListItems(enteredDate: Date, dataItem: any, type: any) {
    if (
      dataItem.checkNbr !== null &&
      dataItem.checkNbr !== undefined &&
      dataItem.checkNbr !== ''
    ) {
    ...
    switch (type.toUpperCase()) {
      case 'DATE_PAYMENT_RECONCILED':
        if (enteredDate > todayDate) {
          ...
          dataItem.datePaymentRecInValidMsg =
            'Reconciled date cannot be a future date.';
        } else {
      ...
      case 'DATE_PAYMENT_SENT':
        if (enteredDate > todayDate) {
          ...
          dataItem.datePaymentSentInValidMsg =
            'Date payment sent cannot be a future date.';
        } else {
  ...
  printAdviceLetterChange(dataItem: any) {
    ...
    if (
      this.selectedReconcileDataRows !== null &&
      this.selectedReconcileDataRows?.length > 0
    ) {
      this.selectedReconcileDataRows =
        this.reconcilePaymentGridUpdatedResult.find(
          (x: any) => x.paymentRequestId === dataItem.paymentRequestId
        );
    }
    this.isRecordForPrint = this.reconcilePaymentGridUpdatedResult.filter(
  ...
  calculateCharacterCountBulkNote(note: any) {
    ...
    let bulkNoteCharactersCount = note ? note.length : 0;
  ...
  validateReconcileGridRecord() {
    this.reconcilePaymentGridUpdatedResult.forEach(
      (currentPage: any, index: number) => {
        if (
          currentPage.checkNbr !== null &&
          currentPage.checkNbr !== '' &&
          currentPage.checkNbr !== undefined
        ) {
          if (
            currentPage.paymentReconciledDate == null ||
            currentPage.paymentReconciledDate === undefined ||
            currentPage.paymentReconciledDate === ''
          ) {
            currentPage.datePaymentRecInValid = true;
            currentPage.datePaymentRecInValidMsg =
              'Date payment reconciled is required.';
          }
          if (
            currentPage.paymentSentDate == null ||
            currentPage.paymentSentDate === undefined ||
            currentPage.paymentSentDate === ''
          ) {
            currentPage.datePaymentSentInValid = true;
            currentPage.datePaymentSentInValidMsg =
              'Date payment sent is required.';
          }
        } else {
  ...
  assignUpdatedItemToPagedList() {
    this.reconcilePaymentGridPagedResult.data.forEach((item: any) => {
      let dataItem = this.reconcilePaymentGridUpdatedResult.find(
        (x: any) => x.paymentRequestId === item.paymentRequestId
      );
  ...
  public onPrintAuthorizationOpenClicked(template: TemplateRef<unknown>): void {
    ...
    const isValid = this.reconcilePaymentGridUpdatedResult.filter(
      (x: any) => x.datePaymentSentInValid || x.datePaymentRecInValid
    );
    const datePaymentSentInValidCount =
      this.reconcilePaymentGridUpdatedResult.filter(
        (x: any) => x.datePaymentSentInValid
    ...
    this.totalCount =
      datePaymentSentInValidCount.length + datePaymentRecInValidCount.length;
    if (isValid.length > 0) {
      this.pageValidationMessageFlag = true;
      this.pageValidationMessage =
        this.totalCount +
      ...
      this.reconcilePaymentGridUpdatedResult.filter(
        (x: any) =>
          x.checkNbr != null && x.checkNbr !== undefined && x.checkNbr !== ''
      ).length <= 0
    ) {
      this.pageValidationMessage = null;
      this.pageValidationMessageFlag = true;
      ...
      this.selectedReconcileDataRows.forEach((data: any) => {
        data.paymentReconciledDate = this.intl.formatDate(
          data.paymentReconciledDate,
          this.dateFormat
        );
        data.paymentSentDate = this.intl.formatDate(
          data.paymentSentDate,
          this.dateFormat
        );
      });
    }
  ...
  onRowSelection(grid: any, selection: any) {
    ...
    this.reconcilePaymentGridUpdatedResult
      .filter(
        (x: any) =>
          x.checkNbr != null &&
          x.checkNbr !== undefined &&
          x.checkNbr !== '' &&
          x.checkNbr === data.checkNbr
      )
      .forEach((item: any) => {
        let object = {
    ...
    const ReconcilePaymentResponseDto = {
      ...
      paymentToReconcileCount:
        data.checkNbr == null || data.checkNbr == undefined ? 0 : 1,
