...
export class ClientEditViewComponent implements OnInit, OnDestroy {
  ...
  private assignModelToForm(applicantInfo: ApplicantInfo) {
    ...
    this.interpreterTypeInputDisable = this.applicantInfo.client?.interpreterCode === 'YES' ? false : true;
    ...
    this.startAgeDeafOrHearingInputDisable = this.applicantInfo.client.deafOrHearingCode === 'YES' ? false : true;
    ...
    this.startAgeBlindSeeingInputDisable = this.applicantInfo.client?.blindSeeingCode === 'YES' ? false : true;
    ...
    this.startAgeWalkingClimbingDifficultyInputDisable = this.applicantInfo.client?.walkingClimbingDifficultyCode === 'YES' ? false : true;
    ...
    this.startAgeDressingBathingDifficultyInputDisable = this.applicantInfo.client?.dressingBathingDifficultyCode === 'YES' ? false : true;
    ...
    this.startAgeConcentratingDifficultyInputDisable = this.applicantInfo.client?.concentratingDifficultyCode === 'YES' ? false : true;
    ...
    this.startAgeErrandsDifficultyInputDisable = this.applicantInfo.client?.errandsDifficultyCode === 'YES' ? false : true;
  ...
  private updateGenderContorlvalues(applicantInfo: ApplicantInfo) {
    ...
    const TransgenderList = [TransGenderCode.YES_F_TO_M.toString(), TransGenderCode.YES_M_TO_F.toString()];
  ...
  private assignRaceAndEthnicityToForm() {
    if (this.applicantInfo?.clientRaceList?.length >= 0 && this.raceAndEthnicity?.length >= 0) {
  ...
  private updateInitialWorkflowCheckList(): void {
    ...
    completedDataPoints = this.updateCompleteData(trans, 'TransgenderDescription', 'transgenderCode', completedDataPoints);

    let gender = this.appInfoForm.controls['BirthGender']?.value;
    completedDataPoints = this.updateCompleteData(gender, 'BirthGenderDescription', 'birthGenderCode', completedDataPoints);
    ...
    completedDataPoints = this.pushCompletedData(interpreter, 'interpreterType', 'interpreter', completedDataPoints);

    let deafOrHearing = this.appInfoForm.controls['deafOrHearingCode']?.value;
    completedDataPoints = this.pushCompletedData(deafOrHearing, 'startAgeDeafOrHearing', 'deafOrHearing', completedDataPoints);

    let blindSeeing = this.appInfoForm.controls['blindSeeingCode']?.value;
    completedDataPoints = this.pushCompletedData(blindSeeing, 'startAgeBlindSeeing', 'blindSeeing', completedDataPoints);

    let walkingClimbing = this.appInfoForm.controls['walkingClimbingDifficultyCode']?.value;
    completedDataPoints = this.pushCompletedData(walkingClimbing, 'startAgeWalkingClimbingDifficulty', 'walkingClimbingDifficulty', completedDataPoints);

    let dressingBathing = this.appInfoForm.controls['dressingBathingDifficultyCode']?.value;
    completedDataPoints = this.pushCompletedData(dressingBathing, 'startAgeDressingBathingDifficulty', 'dressingBathingDifficulty', completedDataPoints);

    let concentrating = this.appInfoForm.controls['concentratingDifficultyCode']?.value;
    completedDataPoints = this.pushCompletedData(concentrating, 'startAgeConcentratingDifficulty', 'concentratingDifficulty', completedDataPoints);

    let errandsDifficulty = this.appInfoForm.controls['errandsDifficultyCode']?.value;
    completedDataPoints = this.pushCompletedData(errandsDifficulty, 'startAgeErrandsDifficulty', 'errandsDifficulty', completedDataPoints);
  ...
  private updateCompleteData(value: any, descCode: string, dataPointName: string, completedDataPoints: any) {
    if (value && (value !== 'NOT_LISTED' || (value === 'NOT_LISTED' && this.appInfoForm.controls[descCode]?.value))) {
      completedDataPoints.push({ dataPointName: dataPointName, status: StatusFlag.Yes });
    }
    return completedDataPoints;
  }

  private pushCompletedData(value: any, descCode: string, dataPointName: string, completedDataPoints: any) {
    if (value && (value !== StatusFlag.Yes || (value === StatusFlag.Yes && this.appInfoForm.controls[descCode]?.value))) {
      completedDataPoints.push({ dataPointName: dataPointName, status: StatusFlag.Yes });
    }
    return completedDataPoints;
  }
  ...
  searchDuplicateClient(fieldname: any) {
    ...
    let firstName = !!this.appInfoForm.controls['firstName'].value ? this.appInfoForm.controls['firstName'].value : '';
    let lastName = !!this.appInfoForm.controls['lastName'].value ? this.appInfoForm.controls['lastName'].value : '';
    let dateOfBirth = this.appInfoForm.controls['dateOfBirth'].value;
    let clientSsn = !!this.appInfoForm.controls['ssn'].value ? this.appInfoForm.controls['ssn'].value : '';
