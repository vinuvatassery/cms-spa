...
export class EmailListComponent implements OnChanges  {
  ...
  private loadClientEmailsList(): void {     
    ...
    this.gridDataHandle()
    this.loadEmails(this.state.skip ?? 0 ,this.state.take ?? 0,this.sortValue , this.sortType ,false)    
  }
  loadEmails(skipcountValue : number,maxResultCountValue : number ,sortValue : string , sortTypeValue : string,showDeactivated : boolean)
   {
  ...
  {  
    ...
    this.addClientEmailResponse$.pipe(first((addResponse: any ) => addResponse != null))
  ...
  {     
   this.subscriptionData =  this.clientEmail$?.pipe(first((emailData: any ) => emailData?.clientEmailId != null))
  ...
  onPreferredEmailClicked(clientEmailId : string)
  {
    ...
    this.preferredClientEmail$.pipe(first((Response: any ) => Response != null))
  ...
  handleAcceptEmailRemove(isDelete :boolean)
   {  
      if(isDelete)
      {
        ...
        this.removeClientEmail$.pipe(first((deleteResponse: any ) => deleteResponse != null))
   ...
   {  
      if(isDeactivate)
      {
        ...
        this.deactivateClientEmail$.pipe(first((deactResponse: any ) => deactResponse != null))
  ...
  onhistorychkBoxChanged()
  {
    ...
    this.loadEmails(this.state.skip ?? 0 ,this.state.take ?? 0,this.sortValue , this.sortType ,this.historychkBoxChecked)    
  }
}
