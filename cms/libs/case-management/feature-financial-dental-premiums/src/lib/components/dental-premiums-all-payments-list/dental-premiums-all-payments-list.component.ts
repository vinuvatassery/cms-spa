...
@Component({
  selector: 'cms-dental-premiums-all-payments-list',
  templateUrl: './dental-premiums-all-payments-list.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DentalPremiumsAllPaymentsListComponent
  implements OnInit, OnChanges
{
  ...
  isDentalPremiumsAllPaymentsGridLoaderShow = false;

  @Input() pageSizes: any;
  @Input() sortValue: any;
  @Input() sortType: any;
  @Input() sort: any;
  @Input() dentalPremiumsAllPaymentsGridLists$: any;
  @Output() loadDentalPremiumsAllPaymentsListEvent = new EventEmitter<any>();
  ...
  gridDentalPremiumsAllPaymentsDataSubject = new Subject<any>();
  gridDentalPremiumsAllPaymentsData$ =
    this.gridDentalPremiumsAllPaymentsDataSubject.asObservable();
  ...
  ngOnInit(): void {
    this.loadDentalPremiumsAllPaymentsListGrid();
  }
  ngOnChanges(): void {
    this.state = {
      skip: 0,
  ...
  private loadDentalPremiumsAllPaymentsListGrid(): void {
  ...
  loadRefundAllPayments(
    ...
    this.isDentalPremiumsAllPaymentsGridLoaderShow = true;
    ...
    this.loadDentalPremiumsAllPaymentsListEvent.emit(gridDataRefinerValue);
    this.gridDataHandle();
  }
  ...
  gridDataHandle() {
    this.dentalPremiumsAllPaymentsGridLists$.subscribe(
      (data: GridDataResult) => {
        ...
        this.gridDentalPremiumsAllPaymentsDataSubject.next(
          this.gridDataResult
        );
        if (data?.total >= 0 || data?.total === -1) {
          this.isDentalPremiumsAllPaymentsGridLoaderShow = false;
        }
      }
    );
    this.isDentalPremiumsAllPaymentsGridLoaderShow = false;
