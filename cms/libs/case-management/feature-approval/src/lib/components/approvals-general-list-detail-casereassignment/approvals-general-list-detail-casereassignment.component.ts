...
@Component({
  ...
  templateUrl:
    './approvals-general-list-detail-casereassignment.component.html',
})
  ...
  ngOnInit(): any {
    if (this.approvalId) {
      ...
      this.pendingApprovalGeneralFacade
        .loadCasereassignmentExpandedInfo(this.approvalId)
        .subscribe({
          next: (response) => {
          ...
          error: (err) => {
            this.pendingApprovalGeneralFacade.showHideSnackBar(
  ...
  onClientNameClicked() {
    if (this.casereassignmentExpandedInfoData?.clientId) {
      this.router.navigate([
        `/case-management/cases/case360/${this.casereassignmentExpandedInfoData?.clientId}`,
      ]);
    }
  }
}
